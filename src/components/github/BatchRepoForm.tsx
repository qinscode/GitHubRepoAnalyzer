  ...
  const handleSubmit = async (event: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      for (const url of urlList) {
        ...
        if (items.some((repo) => repo.url.toLowerCase() === trimmedUrl.toLowerCase())) {
      ...
      for (let index = 0; index < items.length; index++) {
        ...
        try {
          ...
          console.error(`Error processing ${items[index]?.url || 'unknown repository'}:`, error_);
          ...
          <Box className="form-container" component="form" onSubmit={handleSubmit}>
            <Box sx={{ position: 'relative', mb: 3 }}>
              <TextField
                ...
                onChange={(event_) => { setRepoUrls(event_.target.value); }}
              />
            </Box>
              ...
              <Box sx={{ mb: 3 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                  <Typography color="text.secondary" variant="body2">
                    Processing repository {currentIndex + 1} of {repoItems.length}
                  </Typography>
                  <Typography color="text.secondary" variant="body2">
                    {Math.round(progress)}%
                  </Typography>
            ...
            {repoItems.length > 0 && (
              ...
              >
                <Typography sx={{ mb: 1.5, fontWeight: 600, color: 'text.primary' }} variant="subtitle2">
                  Repository Status
                </Typography>
                <Stack spacing={1}>
                  {repoItems.map((item) => (
            ...
            >
              ...
              <Typography variant="caption">
                Batch processing allows you to analyze multiple repositories at once.
              </Typography>
            </Box>
          </Box>
        </CardContent>
