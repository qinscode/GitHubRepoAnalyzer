...
const AllContributorsCommitChart = ({
  ...
  const chartData = useMemo(() => {
    ...
    // Filter out any contributors with missing data
    const validContributors = contributorStats.filter(
      contrib => contrib && contrib.author && contrib.author.login && contrib.weeks
    );
  ...
  const contributorColors = useMemo(() => {
    ...
    const validContributors = contributorStats.filter(
      contrib => contrib && contrib.author && contrib.author.login
    );
    ...
    validContributors.forEach((contributor, index) => {
      colors[contributor.author.login] = generateColor(index, validContributors.length);
    });
  ...
  if (chartData.length === 0 || !contributorStats || contributorStats.length === 0) {
    return (
      <Box sx={{ 
        alignItems: 'center',
        display: 'flex', 
        ...
        >
          ...
          {validContributors.map(contributor => (
            <Line
              ...
              stroke={contributorColors[contributor.author.login] || primaryColor}
              strokeWidth={2}
              dot={{ r: 3 }}
              activeDot={{ r: 5 }}
            />
